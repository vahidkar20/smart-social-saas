        ...
        if(m) {
            ...
            if(window.messengerData && Array.isArray(window.messengerData)) {
                ...
                if(messenger) {
                    ...
                    if(document.getElementById('edit_messenger_active')) document.getElementById('edit_messenger_active').checked = (messenger.is_active == 1);
                }
            }
...
js = js.replace(/let editMessengerTarget = e\.target\.closest \? e\.target\.closest\('\.btn-edit-messenger'\) \: \(e\.target\.classList\.contains\('\.btn-edit-messenger'\) \? e\.target \: null\);[\s\S]*?m\.style\.display = 'flex';\n        }\n    }/m, replacement);
fs.writeFileSync('assets/js/portal-core.js', js);
console.log('patched messenger edit fields correctly');
