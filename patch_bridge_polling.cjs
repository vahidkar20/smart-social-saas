...
const oldCode1 = `window.startBridgePolling = function(taskId, onComplete, onTimeout) {
    let attempts = 0;
    let maxAttempts = 60; // 2.5 minutes max
    let pollInterval = setInterval(function() {
        attempts++;
        if (attempts > maxAttempts) {
            clearInterval(pollInterval);
            showToast('زمان انتظار پاسخ چت‌بات به پایان رسید', 'warning');
            ...
            .then(res => {
                if (res.success) {
                    if (res.data.status === 'completed') {
                        ...
                        clearInterval(pollInterval);
                        showToast('وظیفه منقضی یا لغو شد', 'error');
                        if (onTimeout) onTimeout();
                    }
                }
            })
...
window.startBridgePolling = function(taskId, onComplete, onTimeout) {
    ...
    window._bridgePollInterval = setInterval(function() {
        ...
        attempts++;
        if (attempts > maxAttempts) {
            setBridgeModalState(2, 'زمان انتظار به پایان رسید. چت‌بات در زمان مقرر پاسخی نداد یا صفحه مسدود شده است.');
            if (onTimeout) onTimeout();
            return;
        }
...
if (js.includes('window.startBridgePolling = function(taskId, onComplete, onTimeout) {')) {
    js = js.replace(oldCode1, newCode1);
    fs.writeFileSync('assets/js/portal-core.js', js);
    console.log('Patched startBridgePolling');
} else {
    console.log('Could not find startBridgePolling code');
}
