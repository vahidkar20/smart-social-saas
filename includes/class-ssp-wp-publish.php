...
trait SSP_WpPublish {
    ...
    private function get_user_product_templates($user_id) {
        return is_array($__tmp = get_user_meta($user_id, 'ssp_product_templates', true)) ? $__tmp : [];
    }
    ...
    private function get_user_publish_history($user_id) {
        return is_array($__tmp = get_user_meta($user_id, 'ssp_publish_history', true)) ? $__tmp : [];
    }
    ...
    private function get_product_ai_prompt($content_type, $name, $brief, $prompt_mode, $custom_prompt) {
        ...
        if (strpos($prompt_mode, 'saved_') === 0) {
            ...
            $saved_templates = is_array($__tmp = get_user_meta($user_id, 'ssp_prompt_templates', true)) ? $__tmp : [];
        ...
        } elseif ($content_type === 'article' || $content_type === 'page') {
            $key = 'default_post';
        } elseif ($content_type === 'post' || $content_type === 'social') {
            $key = 'default_social_post';
        } else {
            $key = 'default_post';
        }
    ...
    private function build_prompt_from_saved_template($template, $product_name, $product_brief = '') {
        ...
        if ($is_article) {
            ...
            $json_format = <<<'JSON'
دستور اکید: خروجی شما باید ۱۰۰٪ منحصراً یک آبجکت معتبر JSON خالص بدون هیچ کاراکتر اضافی، بدون پیش‌گفتار، بدون پس‌گفتار و بدون هیچ متن توضیحی باشد:
{
  "title": "عنوان جذاب، ترغیب‌کننده و کاملاً سئو شده برای مقاله (حداکثر ۷۰ کاراکتر)",
  "content": "محتوای کامل و غنی مقاله با ساختار حرفه‌ای HTML شامل تگ‌های <h2>، <h3>، <p>، <ul><li>، <strong> و جداول در صورت نیاز (بدون کاراکترهای بک‌تیک و مارک‌داون)",
  "excerpt": "خلاصه ۲ تا ۳ جمله‌ای بسیار جذاب و ترغیب‌کننده برای خلاصه مقاله",
...
دستور اکید: خروجی شما باید ۱۰۰٪ منحصراً یک آبجکت معتبر JSON خالص بدون هیچ کاراکتر اضافی، بدون پیش‌گفتار، بدون پس‌گفتار و بدون هیچ متن توضیحی باشد:
{
  ...
  "short_description": "توضیحات کوتاه، جذاب و ترغیب‌کننده در ۲ تا ۳ جمله",
  "description": "توضیحات بسیار جامع با ساختار حرفه‌ای HTML شامل <h2>معرفی و بررسی محصول</h2>، <p>متن</p>، <h3>ویژگی‌ها و مزایای کلیدی</h3><ul><li>ویژگی ۱</li><li>ویژگی ۲</li></ul>، <h3>مشخصات فنی</h3>، <h3>راهنمای استفاده و شرایط گارانتی</h3>",
  "regular_price": "قیمت اصلی به عدد تومان مثلا 450000",
  "sale_price": "قیمت با تخفیف به عدد تومان یا خالی",
  "categories": ["دسته‌بندی ۱", "دسته‌بندی ۲"],
        ...
        $product_fields = <<<'JSON'
{
  "name": "نام دقیق، تجاری و سئو شده محصول به فارسی (حداکثر ۶۰ کاراکتر)",
  "sku": "کد انبار انگلیسی یکتا مانند PRD-8520",
  "short_description": "۲ تا ۳ جمله جذاب و ترغیب‌کننده برای خلاصه محصول",
  "description": "محتوای جامع با ساختار استاندارد HTML: <h2>معرفی محصول</h2><p>متن معرفی</p><h3>ویژگی‌های برجسته</h3><ul><li>ویژگی ۱</li><li>ویژگی ۲</li><li>ویژگی ۳</li></ul><h3>مشخصات فنی</h3><p>اطلاعات فنی کامل</p><h3>راهنمای استفاده و گارانتی</h3><p>توضیحات تکمیلی</p>",
  "regular_price": "قیمت اصلی به عدد تومان مثلا 380000",
  "sale_price": "قیمت تخفیف به تومان یا خالی",
  "categories": ["دسته مرتبط ۱", "دسته مرتبط ۲"],
  "tags": ["تگ ۱", "تگ ۲", "تگ ۳"],
  "attributes": [{"name": "رنگ", "options": ["مشکی", "سفید"]}],
  "weight": "0.5",
  "meta_title": "تیتر سئو حداکثر ۶۰ کاراکتر",
  "meta_description": "متا دیسکریپشن سئو حداکثر ۱۵۰ کاراکتر"
}
JSON;
        $post_fields = <<<'JSON'
{
  "title": "عنوان جذاب، گیرا و سئو شده برای مقاله وبلاگ (حداکثر ۷۰ کاراکتر)",
  "content": "محتوای کامل و سئو شده با ساختار HTML: <h2>مقدمه و طرح مسئله</h2><p>متن مقدمه</p><h2>بررسی جامع و نکات کلیدی</h2><p>توضیحات کامل</p><ul><li>نکته کاربردی ۱</li><li>نکته کاربردی ۲</li></ul><h2>راهکارها و جزئیات اجرایی</h2><p>متن تکمیلی</p><h2>نتیجه‌گیری و جمع‌بندی</h2><p>نتیجه و دعوت به اقدام</p>",
  "excerpt": "خلاصه جذاب و خواندنی ۲ تا ۳ جمله‌ای برای پیش‌نمایش مقاله",
...
JSON;
        $social_post_fields = <<<'JSON'
{
  "title": "عنوان جذاب و قلاب کنجکاوی برای پست شبکه‌های اجتماعی",
  "message": "متن کامل، روان و بسیار جذاب پست با رعایت دقیق پاراگراف‌بندی، لحن صمیمی و حرفه‌ای، ایموجی‌های مناسب، فهرست‌بندی بولت‌وار با ایموجی، و دعوت به اقدام (CTA) بسیار شفاف و ترغیب‌کننده در انتهای متن",
  "hashtags": "#هشتگ۱ #هشتگ۲ #هشتگ۳ #هشتگ۴ #هشتگ۵"
}
JSON;

        return [
            'default_product' => "تولید محصول حرفه‌ای و کامل WooCommerce به زبان فارسی.\nنام محصول: {name}\nتوضیحات، مشخصات و نکات کلیدی: {brief}\n\n" .
                "الزامات محتوا:\n- کاملاً واقعی، تخصصی، کاربردی و ترغیب‌کننده برای تصمیم‌گیری خرید\n- دارای مشخصات فنی ملموس و ویژگی‌های متمایز\n- ساختار HTML شیک و تمیز با تگ‌های h2, h3, p, ul/li, strong (بدون استفاده از بک‌تیک)\n- قیمت و مشخصات معقول و متناسب با بازار ایران به تومان\n\n" .
                "دستور اکید: پاسخ شما باید ۱۰۰٪ منحصراً یک آبجکت معتبر JSON بدون هیچ کاراکتر یا متن اضافی قبل یا بعد از آن باشد:\n{$product_fields}",
            'default_post' => "تولید مقاله تخصصی و پست وبلاگ وردپرس به زبان فارسی.\nموضوع مقاله: {name}\nتوضیحات، سرفصل‌ها و نکات تکمیلی: {brief}\n\n" .
                "الزامات محتوا:\n- کاملاً آموزشی، عمیق، کاربردی و جذاب برای مخاطبان\n- رعایت اصول سئو و استفاده از کلمات کلیدی مرتبط\n- ساختار منظم HTML استاندارد شامل تگ‌های h2, h3, p, ul/li, strong\n- نتیجه‌گیری قوی و دعوت به اقدام در پایان مقاله\n\n" .
                "دستور اکید: پاسخ شما باید ۱۰۰٪ منحصراً یک آبجکت معتبر JSON بدون هیچ کاراکتر یا متن اضافی قبل یا بعد از آن باشد:\n{$post_fields}",
            'default_social_post' => "تولید پست جذاب، پربازده و حرفه‌ای برای شبکه‌های اجتماعی و پیام‌رسان‌ها (تلگرام، بله، ایتا و اینستاگرام) به زبان فارسی.\nموضوع یا ایده پست: {name}\nتوضیحات و نکات تکمیلی: {brief}\n\n" .
                "الزامات محتوا:\n- قلاب ذهنی (Hook) بسیار گیرا در خط اول\n- متن جذاب، پرکشش، خوانا و با فاصله‌گذاری مناسب بین پاراگراف‌ها\n- استفاده هدفمند از ایموجی‌های جذاب\n- دعوت به اقدام (Call to Action) شفاف در انتهای متن\n- ۵ الی ۷ هشتگ هدفمند و پرمخاطب\n\n" .
                "دستور اکید: پاسخ شما باید ۱۰۰٪ منحصراً یک آبجکت معتبر JSON بدون هیچ کاراکتر یا متن اضافی قبل یا بعد از آن باشد:\n{$social_post_fields}",
            'fashion' => "محصول مد، پوشاک و استایل به زبان فارسی.\nنام محصول: {name}\nتوضیحات: {brief}\n\n" .
                "تمرکز: استایل کاربردی، جنس پارچه، فیت و قواره، راهنمای شستشو و موقعیت‌های استفاده\nویژگی‌ها: سایز، رنگ، جنس، نوع دوخت\ndescription با ساختار کامل HTML. خروجی فقط و فقط JSON معتبر:\n{$product_fields}",
            'electronics' => "محصول دیجیتال، گجت و لوازم الکترونیکی به زبان فارسی.\nنام محصول: {name}\nتوضیحات: {brief}\n\n" .
                "تمرکز: مشخصات فنی دقیق، پردازشگر و باتری، کیفیت ساخت، سازگاری و گارانتی\nویژگی‌ها: برند، مدل، رنگ، اتصالات\ndescription با ساختار کامل HTML. خروجی فقط و فقط JSON معتبر:\n{$product_fields}",
            'food' => "محصول مواد غذایی، خوراکی و نوشیدنی به زبان فارسی.\nنام محصول: {name}\nتوضیحات: {brief}\n\n" .
                "تمرکز: تازگی مواد اولیه، طعم و عطر، ارزش غذایی، شرایط نگهداری و نشان سلامت\nویژگی‌ها: وزن، ترکیبات، تاریخ انقضا، شیوه نگهداری\ndescription با ساختار کامل HTML. خروجی فقط و فقط JSON معتبر:\n{$product_fields}",
            'health_beauty' => "محصول آرایشی، بهداشتی، پوستی و مراقبتی به زبان فارسی.\nنام محصول: {name}\nتوضیحات: {brief}\n\n" .
                "تمرکز: ترکیبات کلیدی فعال، نتایج بالینی و مزایای پوستی، روش استفاده صحیح، اصالت محصول\nویژگی‌ها: برند، حجم، نوع پوست و مو، محفظه\ndescription با ساختار کامل HTML. خروجی فقط و فقط JSON معتبر:\n{$product_fields}",
            'home_garden' => "محصول خانه، دکوراسیون، لوازم خانگی و آشپزخانه به زبان فارسی.\nنام محصول: {name}\nتوضیحات: {brief}\n\n" .
                "تمرکز: ابعاد و جنس متریال، ماندگاری و استحکام، هماهنگی با دکوراسیون، نحوه نگهداری و شستشو\nویژگی‌ها: متریال، ابعاد، رنگ، وزن\ndescription با ساختار کامل HTML. خروجی فقط و فقط JSON معتبر:\n{$product_fields}",
            'sports' => "محصول ورزشی، تناسب اندام و کوهنوردی به زبان فارسی.\nنام محصول: {name}\nتوضیحات: {brief}\n\n" .
                "تمرکز: ارگونومی و راحتی، دوام در فشارهای ورزشی، مناسب چه رشته‌ها و تمریناتی، ویژگی‌های ایمنی\nویژگی‌ها: برند، سایز، جنس، استانداردهای ساخت\ndescription با ساختار کامل HTML. خروجی فقط و فقط JSON معتبر:\n{$product_fields}",
        ];
    }
}
